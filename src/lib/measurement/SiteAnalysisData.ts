...
import { AnalysisDetail, AnalysisResult, SitesEntry } from "../model";
...
export class SiteAnalysisData {
  ...
  static async fromFile(outputPath: string, sitesEntry: SitesEntry) {
    const { site, failureErrorEntries } = sitesEntry;
    const entry = async (
      browserId: BrowserId,
      index: number,
      runId: RunId
    ): Promise<SiteAnalysisDataEntry | null> => {
      if (
        failureErrorEntries.find((entry) => entry.browserId === browserId)!
          .failureError !== null
      ) {
        return null;
      }
    ...
    return new SiteAnalysisData(
      site,
      (await Promise.all(entries)).filter(isNonNullable)
    );
  }
}
